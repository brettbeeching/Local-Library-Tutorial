...
def index(request):
    ...
    num_genres = Genre.objects.all().annotate(num_books=Count('name'))

    WORD = 'Science'

    book_with_word = Book.objects.filter(title__icontains=WORD).count()
    genre_with_word = Genre.objects.filter(name__icontains=WORD).count()
    ...
    context = {
        ...
        'book_with_word': book_with_word,
        'genre_with_word': genre_with_word,
        'WORD': WORD,
